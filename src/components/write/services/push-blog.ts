...
export async function deleteBlog(slug: string): Promise<void> {
    ...
    try {
        ...
        // 2. 【修复关键】获取当前 Commit 对应的 Tree SHA
        ...
        // 3. 【修复关键】这里必须使用 baseTreeSha，而不是 latestCommitSha
        ...
        // 创建提交时，parents 依然是 latestCommitSha，这是对的
        const commitDataResult = await createCommit(token, GITHUB_CONFIG.OWNER, GITHUB_CONFIG.REPO, `删除文章: ${slug}`, treeData.sha, [latestCommitSha])

        toast.loading('正在更新分支...', { id: toastId })
        await updateRef(token, GITHUB_CONFIG.OWNER, GITHUB_CONFIG.REPO, `heads/${GITHUB_CONFIG.BRANCH}`, commitDataResult.sha)
